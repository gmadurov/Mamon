  ...
  return (
    <Card>
      <Text>
        ...
        For a {purchase?.balance ? "payed" : "loged"} total of €{total?.toFixed(2).toString()} on {created.toDateString()}{" "}
        {created.toLocaleTimeString("nl-NL")}:
      </Text>
      {purchase?.orders?.map((order) => (
        <View key={"cart product" + order.product}>
          <Text>
            {order.quantity} {products?.find((product) => product.id === order.product)?.name}
            {order.quantity > 1 && "s"}
          </Text>
        </View>
      ))}
